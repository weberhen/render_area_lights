...
obj_ply = 'debug/armadillo.ply'
obj_scale = '3'
scene = 'debug/scene_area_light'
...
print(command)
os.system(command)
os.system('mtsutil tonemap -m 10 -o debug/scene_area_lights.png debug/scene_area_light.exr')
...
iblN = ibl.cpu().numpy()
hdrio.imsave("debug/envmap.exr", iblN.astype('float16'))

scene = '-Denvmap=debug/envmap.exr debug/scene_pano' #TODO remove this ugly hack
command = create_mitsuba_command(obj_pos, obj_ply, obj_scale, posCenters, radius, intensities, ambients, depths, scene)
print(command)
os.system(command)
os.system('mtsutil tonemap -m .1 -o debug/scene_pano.png debug/scene_pano.exr')